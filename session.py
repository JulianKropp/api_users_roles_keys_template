...
USER_SESSION_KEY     = "sessions:{user_id}:sessions:{session_id}"
APIKEY_SESSION_KEY   = "sessions:{user_id}:api_keys:{apikey_id}:sessions:{session_id}"
...
class SessionAPIKey(Session):
    apikey_id: str = ""
    user_id:   str = ""         # <- we need the owning user for key building
...
class SessionManager:
    ...
    async def _keys_for_session_id(self, session_id: str) -> List[str]:
        ...
        """
        patterns = [
            f"sessions:*:sessions:{session_id}",
            f"sessions:*:api_keys:*:sessions:{session_id}",
        ]
        keys: List[str] = []
    ...
    async def get_sessions(self) -> Dict[str, Union[SessionUser, SessionAPIKey]]:
        ...
        # user-sessions
        async for key in self.redis.scan_iter(match="sessions:*:sessions:*"):
        ...
        # API-key-sessions
        async for key in self.redis.scan_iter(match="sessions:*:api_keys:*:sessions:*"):
            raw = await self.redis.get(key)
            if raw:
    ...
    async def get_sessions_by_user(self, user_id: str) -> List[SessionUser]:
        pattern = f"sessions:{user_id}:sessions:*"
    ...
    async def get_sessions_by_apikey(self, apikey_id: str) -> List[SessionAPIKey]:
        pattern = f"sessions:*:api_keys:{apikey_id}:sessions:*"
...
async def test_session_manager() -> None:
    ...
    print(f"Created sessions: {session._id}, user_id: {session.user_id}, roles: {user.roles}")
